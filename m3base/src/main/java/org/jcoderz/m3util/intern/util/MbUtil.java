...
public final class MbUtil {
    ...
    /**
     * Checks if there are indications that the given release group is of type
     * soundtrack. Supports the 'New' MB secondary-type-list.
     *
    ...
    /**
     * Checks if there are indications that the given release group is of type
     * story (either Spokenword, Audiobook, or Interview). Supports the 'New' MB
     * secondary-type-list.
     *
     * @param rg the release group to be examined.
     * @return true for release groups of type story.
    ...
    /**
     * This method tries to handle the situation where the track id changed
     * within musicbrainz database. This can happen if releases are merged and
     * happens quite frequently in the ngs schema of musicbrainz, where the same
     * song in different albums appears with the same track id.
     *
     * @param mbClient the connection to the musicbrainz server used if lookups
     * are needed.
     * @param mbData the metadata of the song we take care for
     * @param album the album in which the song (mbData) is expected to be
     * @return the TrackData object containing Release, Medium and Track
     * information uniquely identifying the song.
     */
    public static TrackData getTrackDataWithIdUpdate(
        ...
        if (track.getMedium() == null) {   // still not found, search for alternative / updated track id.
    ...
    private static TrackData findTrackDataByRecording(MbClient mbClient,
        ...
        final Recording recording = mbClient.getRecording(fileId, Collections.<Includes>emptySet());
