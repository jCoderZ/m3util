...
/**
 * This class implements the logic needed to update and freshen a existing well
 * maintained m3dditiez repository. It will update data changes coming from
 * musicbrainz and can also be used to merge a set of clean mp3s into a existing
 * repository. It does not: <ul> <li>Clean up files by id guessing</li>
 * <li>...</li> </ul>
 *
...
public final class RefreshRepository {
     ...
     *
     * @param dryRun if true no actual files will be updated, only a log file
     * will be written.
     * @param dir the directory to work on. Might be a sub-directory of the
     * repos.
    ...
    public RefreshRepository(boolean dryRun, File dir, File repositoryPath, String mbServerHostname) {
        ...
        mDirTreeWalker = new DirTreeWalker(dir, new DirTreeListener() {
            ...
            final List<MusicBrainzMetadata> mFiles = new ArrayList<MusicBrainzMetadata>();
    ...
    private void checkPartialAlbum(List<MusicBrainzMetadata> songs) {
        ...
        try {
            ...
            final Release release = StringUtil.isEmptyOrNull(currentAlbum) ? null : mMusicBrainz.getRelease(currentAlbum);
            final TrackData track = MbUtil.getTrackDataWithIdUpdate(mMusicBrainz, songs.get(0), release);

            final int fullTrackCount = track.getMedium() == null ? 0 : track.getMedium().getTrackList().getCount().intValue();
    ...
    private void moveToNewFilename(FileLocation fl, final MusicBrainzMetadata file)
            throws Exception {
        final String move = file.getFile().getName() + " -> " + fl.getFilename() + " @ "
    ...
    /**
     * Check in which directory the given file should reside. Returns null if
     * the file is already at the right place.
     */
...
//      }
    ...
    private void refineAlbumPos(
        ...
        if (filePos.getFile().getParentFile().equals(mb.getFile().getParentFile())) {
            ...
            final List<MusicBrainzMetadata> existing = readSongs(filePos.getDir());
            if (existing.isEmpty()) {
                logger.log(Level.WARNING,
    ...
    private void refineSinglePos(MusicBrainzMetadata mb, FilePos filePos) {
        ...
        if (!dupe && filePos.getFile().exists()) {
            ...
            MusicBrainzMetadata existingMb = new MusicBrainzMetadata(filePos.getFile());
            if (!existingMb.getFileId().equals(mb.getFileId())) {  // is the not the same song, so try to make filename more qualifying
    ...
    private boolean existAsFullAlbum(MusicBrainzMetadata mb) {
        ...
        if (songFile.exists()) {
            final MusicBrainzMetadata existingMb = new MusicBrainzMetadata(songFile);
        ...
        for (Release rel : recording.getReleaseList().getRelease()) {
            ...
            for (File cand : locations) {
                if (cand.exists() && !cand.equals(mb.getFile())) {
                    final MusicBrainzMetadata existingMb = new MusicBrainzMetadata(cand);
                    if (existingMb.getFileId().equals(mb.getFileId())) {
    ...
    /**
     * Returns true, if the album already exists or is subset of an existing
     * album.
     *
     * @param newAlbum the new album to check
     * @return true, if the album already exists or is subset of an existing
     * album.
     */
    ...
    private boolean isRedundant(List<MusicBrainzMetadata> newAlbum,
        ...
        for (MusicBrainzMetadata song : existingAlbum) {
            final Iterator<MusicBrainzMetadata> newSongsIterator = newSongs.iterator();
    ...
    /**
     * Read all songs in the given directory and return the musicbrainz data.
     *
    ...
    private List<File> getExistingAlbumPos(MusicBrainzMetadata mb) {
        ...
        if (album.getParentFile().isDirectory()) {
            ...
            final File[] listFiles = album.getParentFile().listFiles(new FilenameFilter() {
